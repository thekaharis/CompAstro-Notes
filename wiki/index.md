...
- [[Laplace Operator on 2-D x_HI]] — **The first architecture change in the campaign to clear the replicate floor convincingly.** The pole-residue operator in the global slot beats a matched `fourier`/`fourier` control by **−0.0056 to −0.0057 val_l2** at a matched epoch 50 — about 6× the 0.0002–0.0009 seed floor, with **every seed of every arm negative**, versus mixed signs and one-seed-driven means for frequency mixing. Confirmed independently on RMSE (val_rmse 0.1446 vs 0.1741, −17%; x_HI MAE −22%) by the trainer's own report code, and on boundary-band error (−29% total squared error). **The pole budget is irrelevant between 2 and 8** — two poles per axis buy the entire effect — so the gain is the pole-residue *form*, not pole resolution. Two things it is not: a front-sharpness fix (residual error is still concentrated at bubble walls, gradient metrics barely move), and explained — LNO's motivation is non-periodic transients and 2-D $x_\text{HI}$ has no time-like axis. Costs 3–4× a Fourier epoch. No run reached its 100-epoch budget; all were killed by the 8 h wall.
