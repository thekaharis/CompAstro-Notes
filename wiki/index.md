...
- [[LOS-Monotone Theta Key in 3-D]] — **negative so far, pre-registered.** `NOTES-contrast-map.md` §8 registered the stop rule before the data existed; both architectures fired it at epoch 1. `key_jitter` **0.0000–0.0005** (raw per-slice means are already monotone, so the isotonic fit is the identity) ⇒ the key error is **bias, not jitter**, the one case §8.1 says smoothing cannot reach. The binding constraint is `frac_band` **0.5% → 0.1%**, i.e. **2–10 of 2048 refit slices** in the responsive band, so ~4 bins are fitted on a handful of samples; train gain −1.87% → **−24.52%**. More bins do not help (only 8/14 occupied, 5 floored).
