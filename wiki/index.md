...
- [[3-D Operator Matrix Final Results]] — **the campaign result for the 3-D task.** 27 cells, 20-epoch budget, final held-out evaluation on 200 test cones (10^9 voxels). **`sfno/swhno` is within 2.3% of U-FNO's RMSE at 271x fewer parameters** (0.0591 vs 0.0578, 748 k vs 202.9 M); the **Walsh-Hadamard global slot transfers from 2-D to 3-D** and the SIREN-generated variant is the best global operator tested. Parameter count no longer predicts wall-clock (window loop and SIREN kernel regeneration dominate; the wavelet local slot costs 460 min/epoch for last place). Auxiliary losses cost val_l2 in the *same order across four architectures* (plain < hybrid < bsd < expwall) yet deliver on their own axes: **hybrid halves the U-FNO front width, 16.32 -> 8.72 Mpc, for 4% of val_l2**. **Four metric families produce four different leaders** — RMSE (U-FNO), bubble size (`cnn/swhno`, within 2% at every stage), $P(k)$ amplitude (`swhno/swhno`, 6.5% error) and $P(k)$ coherence (U-FNO) — and every model carries a **+0.19 bias at true $x_\text{HI}\approx0.05$**. Parameter counts corrected to count complex Fourier weights as two reals. **Inference timing (§2.1) qualifies the size headline**: parameters predict cost not at all (Pearson 0.011 against throughput), `sfno/swhno` is the *second slowest* model despite being 271x smaller, the global operator is free (`fno/fno` and `fno/whno` identical to 0.005% across 14.7 M params), and the speed-accuracy Pareto front is just {`cnn/whno`, U-FNO}.
- [[Phase Coherence and Bubble Size Bias]] — **reanalysis of the matrix eval outputs; revises the §5/§7 reading.** Small-scale **phase** accuracy, not small-scale amplitude, controls the bubble-size distribution: $\langle r(k)\rangle_{k>1}$ predicts MFP bias at **Spearman +0.88** ($R^2 = 0.88$, zero bias at $r \approx 0.73$) and JS divergence at $-0.92$, while the amplitude ratio predicts **nothing** ($-0.27$, $p = 0.49$; partial $+0.35$ once $r$ is controlled). Survives controlling for RMSE ($+0.71$) and for ionized-fraction error ($+0.79$), and is monotone 4/4 within the fixed-`swhno`-global family. **Mechanism:** the MFP estimator is a first-passage statistic, so the shortest obstruction dominates; the per-$k$-bin correlation rises monotonically to **+0.97 at $k = 1.9$**; at $\bar{x}_\text{HI} = 0.4$–$0.6$ every cell gets the ionized *area* right to 1–3% while the bias spans $-0.006$ to $-0.49$ (**fragmentation, not under-ionization**); and the $k^2$-weighted level-crossing rate $\nu = \sigma_1/\sigma_0$ tracks the MFP ratio at $\rho = -0.83$. Caveat: the CNN cells' near-zero bias is partly cancellation — they *under*-produce boundaries ($\nu \approx 0.86$).
- [[Granulometry (BSD) Auxiliary Loss]] — **positive.** The first auxiliary term in the campaign that improves what it targets. Morphological opening as a differentiable stand-in for the MFP bubble-size distribution (Pearson **r = 0.99** against the real estimator, no threshold, works on the raw field). Three seed-matched 20-epoch U-FNO runs: relative bubble-size bias **+0.060 → +0.029 (−51%)** and the best Wasserstein distance, at a cost of **6.5% val_l2 / 4.5% test RMSE**. At `x̄_HI 0.60–0.80` the bias is **+0.0015**. Caveats: JS divergence is marginally *worse* (shape not fixed, only the mean), the large-bubble regime degrades for every loss, and a size spectrum is translation-invariant — a field with every bubble displaced 37 px scores **21× better** than one with wrong sizes, so it cannot run without an L2/expwall anchor.
- [[U-FNO BatchNorm Train-Eval Mismatch]] — **closed, positive.** `ufno_norm=batchnorm` is unusable at batch size 1 on 3-D cubes: identical training loss but **4.6× worse `val_l2`** (0.4829 vs 0.1049), gap growing 4.2×→8.1× and 33% of voxels pinned at hard zero. Root-caused from the checkpoint, not by ablation alone — **31/32 channels in `unet3.conv3_1` have `running_var` below `eps`**, giving a **28.6× train/eval scale discrepancy** in that layer; `momentum=0.1` at batch 1 makes running stats an EMA over ~10 cubes. `UFNO_NORM=groupnorm` turned the worst cell of the matrix into the best completed run (**val_l2 0.0471, val_h1 9.15 in 23.6 h**, vs fno_whno-plain 0.0611 / 10.97 in 53.1 h).
- [[LOS-Monotone Theta Key in 3-D]] — **closed 2026-08-17; negative, pre-registered.** Confirmed on a second architecture (`fno_whno` 0.0689 vs its plain cell 0.0611). `NOTES-contrast-map.md` §8 fixed the stop rule before the data existed. Over a completed 20-epoch ufno run `frac_band` never exceeded **1.2%** and hit **0.0% at epoch 7** — the epoch of the run's *largest* gain — so 0–25 of 2048 refit slices ever land in the responsive band and the gains are the expwall objective, not the map. On matched architecture the map **costs 31% val_l2 / 34% val_h1** vs plain L2 (0.0616/12.26 against [[U-FNO BatchNorm Train-Eval Mismatch]]'s 0.0471/9.15). Two sign traps documented: `train gain` is **negative = better**, and the schedule is **never the identity** — it applies near-floor theta (~0.29) across the saturated z>12 region, ~60% of every cone, where there is nothing to sharpen.
