...
## [2026-07-16] ingest | Local-FNO epoch-38 boundary re-run + parity diagnostic (Binac backup)

**Source**: Binac figure backup (`figures/band_out/`, `figures/parity_out/`, synced 2026-07-05).

**Updated**: [[Windowed Local-FNO U-Net Findings]] — new "Update (2026-07-05 results)" section. Epoch-38 checkpoint (167 test cones, run `4204953`): front width **18.5 vs U-FNO 13.9 Mpc** (truth 3.6) — big improvement from 32.2 vs 10.7 at epoch 20, but the U-FNO reference also moved (eval config differs; compare within-run). Paired-bootstrap boundary-H¹ margin shrank ~8× (+0.0247 → +0.0031) yet stays significantly in the U-FNO's favor (P(localfno better)=0.000). The 10-epoch `LOSS_IONIZED_WALL_WEIGHT` experiment (`4014918`) was the worst checkpoint (+0.0359). New `parity_diagnostic.py` (jobs `4205004/4205005`): in nearly-ionized bins both models over-predict — U-FNO bias ≈ +0.17 (RMSE ≈ 0.30), Local-FNO ≈ +0.21 (RMSE ≈ 0.33); same at z = 6–12. The hedging failure mode is now measured directly and is architecture-independent → strengthens the target-side program ([[Lightcone z_re Map Target]], [[Warped LOS Grid Plan]]).

**Indexes/cache updated**: `wiki/hot.md` (Local-FNO active thread replaced with the verdict).

---

## [2026-07-16] synthesis | lightcone-regeneration LaTeX report created

**Output**: [[Chronological Research Report]] — a figure-complete report focused exclusively on regenerating 21cmFAST neutral-fraction lightcones from ground-truth density fields and simulation parameters. It covers the FNO/U-FNO, SirenFNO, and Local-FNO experiments and the current smooth-target and warped-LOS-grid programme.

**Artifacts**:
- `wiki/thesis/report/chronological-research-report.tex`
- `output/pdf/chronological-research-report.pdf`

**Verification**: compiled with XeLaTeX; all 25 pages rendered and visually inspected for figure placement, clipping, tables, headers, and page numbering.
